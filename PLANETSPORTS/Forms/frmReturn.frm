...
Dim aa
        ...
        With ar
            criteria = "Select *From return"
            .Open criteria, strConek, adOpenStatic, adLockOptimistic
            .MoveLast
            
                lastid = Mid(!returnid, 5, Len(!poid))
                lastid = Val(lastid) + 1
                
                If Len(lastid) = 1 Then
                    Me.Caption = "Returned Item - " & "RET-" & "00000" & lastid
                ElseIf Len(lastid) = 2 Then
                    Me.Caption = "Returned Item - " & "RET-" & "0000" & lastid
                ElseIf Len(lastid) = 3 Then
                    Me.Caption = "Returned Item - " & "RET-" & "000" & lastid
                ElseIf Len(lastid) = 4 Then
                    Me.Caption = "Returned Item - " & "RET-" & "00" & lastid
                ElseIf Len(lastid) = 5 Then
                    Me.Caption = "Returned Item - " & "RET-" & "0" & lastid
                Else
                    Me.Caption = "Returned Item - " & "RET-" & lastid
                End If
            .Close
        End With
        
        id = Mid(Me.Caption, 17, 15)
...
On Error Resume Next
    ...
    Call dbconek
    With ar
        .Open "Select *From return", strConek, adOpenStatic, adLockOptimistic
            .AddNew
                !returnid = id
                !supplierid = lblsup
                !supplierdesc = cbox.Text
                !datereturned = Format(Date, "mm/dd/yyyy")
                !returnedby = currentemmsuser
                !Memo = txt(3)
                !printed = "NO"
            .Update
        .Close
    ...
    For i = 1 To lst.ListItems.Count
        With ar
            .Open "Select *From returndetails", strConek, adOpenStatic, adLockOptimistic
                .AddNew
                    !returnid = id
                    !upc = lst.ListItems(i).Text
                    !Description = lst.ListItems(i).SubItems(1)
                    !unitcost = lst.ListItems(i).SubItems(2)
                    !quantity = lst.ListItems(i).SubItems(3)
        ...
        With ar
            .Open "Select *From itemmaster Where upc='" & lst.ListItems(i).Text & "'", strConek, adOpenStatic, adLockOptimistic
                    !stock = !stock - lst.ListItems(i).SubItems(3)
                .Update
            .Close
        End With
    ...
    If A = vbYes Then
        Printer.Print ""
        Printer.Print "RETURN NUMBER: " & id
        Printer.Print "SUPPLIER ID: " & lblsup
        Printer.Print "SUPPLIER NAME: " & cbox.Text
        Printer.Print "DATE RETURNED: " & Format(Date, "MM/DD/YYYY")
        ...
        Printer.Print "----------------------------------------------------------------------------------------------------"
        Printer.Print ""
        Printer.Print "RETURNED BY: " & currentemmsuser
        Printer.Print "NOTE: "
        Printer.Print "      " & txt(3)
    Else
        Open App.Path & "\Return.txt" For Append As #1
            Print #1, " "
