...
Begin VB.Form frmSSS 
   ...
   Begin VB.Label Label1 
      BackColor       =   &H00C0C0C0&
      Caption         =   "SSS Brackets and Monthly Deduction"
      BeginProperty Font 
         Name            =   "Arial"
         Size            =   9.75
         Charset         =   0
...
On Error Resume Next
    ...
    Case "&Update"
        ...
        For i = 0 To 3
            ...
            End If
        Next
        ar.Open "Select *From ssstable Where salarybracket='" & txt(0) & "'", strConek, adOpenStatic, adLockOptimistic
                'ar!salarybracket = txt(0)
                ar!deduction = Format(CCur(txt(1)), "###,###,##0.00")
                ar!salaryfrom = Format(CCur(txt(2)), "###,###,##0.00")
                ar!salaryto = Format(CCur(txt(3)), "###,###,##0.00")
...
Select Case Index
    Case 0
        If KeyAscii = 13 Then
            Call dbconek
            ar.Open "Select *From ssstable Where salarybracket='" & txt(0).Text & "'", strConek, adOpenStatic, adLockOptimistic
